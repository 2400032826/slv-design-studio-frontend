...
export default function GoogleDriveImagePicker({
  ...
  const handleRegisterImage = (e) => {
    // Explicitly prevent default to never submit any parent form
    if (e) {
      e.preventDefault();
      e.stopPropagation();
    }
    ...
    if (!rawInput) {
      setInputError('Please paste a Google Drive share link or File ID.');
      return;
    }
    ...
    if (!fileId) {
      setInputError(
        'Invalid Google Drive link. Please use a link like: https://drive.google.com/file/d/FILE_ID/view?usp=sharing'
      );
      toast.error('Could not extract Google Drive File ID from link');
      return;
    }

    // Check if already registered in current list
    const isAlreadyAdded = selectedImages.some(
      (img) => (img.googleDriveFileId || extractDriveFileId(img.url || img)) === fileId
    );

    if (isAlreadyAdded) {
      toast.error('This Google Drive image is already registered for this product.');
      return;
    }

    const isFirst = selectedImages.length === 0;
    const directExportUrl = `https://drive.google.com/uc?export=view&id=${fileId}`;
    const cdnDisplayUrl = `https://lh3.googleusercontent.com/d/${fileId}=w1200`;
    const cdnThumbUrl = `https://lh3.googleusercontent.com/d/${fileId}=w400`;
    ...
    setDriveUrlInput('');
    toast.success('Google Drive image registered successfully! 📸');
  };

  // Set designated image as the Cover / Main Image
    ...
    <div className="bg-pink-50/50 dark:bg-charcoal-900/60 border border-pink-200/80 dark:border-charcoal-700 rounded-3xl p-4 sm:p-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-pink-100 dark:border-charcoal-800">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-pink-500 to-rose-500 flex items-center justify-center text-white shadow-soft flex-shrink-0">
            <ImageIcon className="w-5 h-5" />
          </div>
          <div>
            <h3 className="font-display font-bold text-sm sm:text-base text-[#1F2937] dark:text-white flex items-center gap-2">
              Google Drive Image Links
              <span className="badge badge-soft text-[10px] font-bold uppercase tracking-wider text-pink-600">
                Direct Share
              </span>
            </h3>
            <p className="text-[11px] text-[#64748B] dark:text-gray-400 mt-0.5">
              Paste share links with <span className="font-semibold text-pink-600">"Anyone with the link → Viewer"</span>
            </p>
          </div>
        </div>

        {selectedImages.length > 0 && (
          <span className="text-xs font-bold text-pink-600 bg-pink-100/70 dark:bg-pink-950/40 px-3 py-1 rounded-full w-fit">
            {selectedImages.length} Image{selectedImages.length > 1 ? 's' : ''} Registered
          </span>
        )}
      </div>

      {/* Share Link Input Form Bar */}
      <div className="space-y-2">
        <label className="block text-xs font-bold uppercase tracking-wider text-[#1F2937] dark:text-gray-300">
          Paste Google Drive Image Share Link
        </label>
        ...
        <div className="flex flex-col sm:flex-row gap-2.5">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="https://drive.google.com/file/d/1p2C_.../view?usp=sharing"
        ...
        {inputError ? (
          ...
          <p className="text-[10px] text-[#64748B] dark:text-gray-400">
            Tip: You can paste multiple Google Drive links one after another and select which one is the cover image.
          </p>
        )}
      </div>

      {/* Registered Images Strip / Previews */}
      {selectedImages.length > 0 ? (
        <div className="bg-white dark:bg-[#1F2937] p-3.5 sm:p-4 rounded-2xl border border-pink-200/80 dark:border-charcoal-700 shadow-soft space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="font-bold text-xs text-[#1F2937] dark:text-white">
                Registered Product Photos ({selectedImages.length})
              </span>
              <span className="text-[10px] text-[#64748B] hidden sm:inline">
                • Click ⭐ to choose the Main Cover Image
              </span>
            </div>
            <button
              type="button"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
            {selectedImages.map((img, index) => {
              const fileId = img.googleDriveFileId || extractDriveFileId(img.url || img.directUrl);
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-2xl border transition-all ${
                    img.isCover
                      ? 'bg-pink-50/80 dark:bg-pink-950/40 border-pink-400 ring-2 ring-pink-400/40 shadow-soft'
                      : 'bg-[#F9FAFB] dark:bg-charcoal-800 border-[#E5E7EB] dark:border-charcoal-700'
                  }`}
                >
                  {/* Thumbnail */}
                  <div className="relative w-16 h-16 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100 border border-[#E5E7EB] dark:border-charcoal-700">
                    ...
                    {img.isCover && (
                      <span className="absolute bottom-1 right-1 bg-gradient-to-r from-pink-500 to-rose-500 text-white text-[8px] font-bold px-1 py-0.5 rounded shadow-sm leading-tight">
                        COVER
                      </span>
                    )}
                  </div>
                  ...
                  <div className="flex-1 min-w-0 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <p className="text-[11px] font-bold text-[#1F2937] dark:text-white truncate font-mono">
                        ID: {fileId ? `${fileId.slice(0, 12)}...` : `Photo #${index + 1}`}
                      </p>
                      {img.isCover && (
                        <span className="text-[10px] font-bold text-pink-600 flex items-center gap-0.5">
                          <Star className="w-3 h-3 fill-pink-500 text-pink-500" /> Main
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className="p-1 rounded-lg text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/40 transition-colors ml-auto"
                        title="Remove Image"
                      >
        ...
        <div className="text-center py-6 px-4 bg-white dark:bg-[#1F2937] rounded-2xl border border-dashed border-pink-200 dark:border-charcoal-700 space-y-2">
          ...
          <p className="text-xs font-semibold text-[#1F2937] dark:text-gray-200">
            No Google Drive images registered for this product yet.
          </p>
          <p className="text-[11px] text-[#64748B] dark:text-gray-400 max-w-md mx-auto">
            Copy the image share link from Google Drive (set to "Anyone with the link → Viewer"), paste it above, and click <strong>REGISTER IMAGE</strong>.
          </p>
        </div>
      )}
    </div>
